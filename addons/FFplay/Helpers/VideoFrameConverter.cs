...
public sealed unsafe class VideoFrameConverter : IDisposable
{
    ...
    private readonly List<byte_ptr4> ptrs = [];

    private readonly List<IntPtr> ptrs2 = [];
    ...
    public AVFrame Convert(AVFrame sourceFrame, int align = -1)
    {
        ...
        if (align < 0)
            for (uint i = 1; i <= 64; i *= 2)
                if (
                    Mathf.Abs(sourceFrame.linesize[0 * i]) % i == 0
        ...
        byte_ptr4 _dstData = new();

        int4 _dstLinesize = new();
